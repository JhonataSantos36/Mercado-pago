...
public class ReviewItemRenderer extends Renderer<ReviewItem> {
    ...
    @Override
    public View render(final ReviewItem component, final Context context, @Nullable final ViewGroup parent) {

        // Cannot use raw inflate because it replaces the old item by the new one, instead of attaching both to the parent
        final View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.mpsdk_review_item, parent, false);
    ...
    private void drawItemFromUrl(final ImageView itemImage, final ReviewItem component, final Context context) {
        int dimen = ScaleUtil.getPxFromDp((int) context.getResources().getDimension(R.dimen.mpsdk_m_height), context);
    ...
    private void drawProductQuantity(final MPTextView itemQuantity, final ReviewItem.Props props, final Context context) {
    ...
    private void drawProductPrice(final MPTextView itemPrice, final ReviewItem.Props props, final Context context) {
        if (props.itemModel.hasToShowPrice()) {
            ...
            if (TextUtils.isNotEmpty(props.unitPriceLabel)) {
                priceText = String.format(Locale.getDefault(), "%s %s", props.unitPriceLabel, props.itemModel.getPrice());
            } else {
